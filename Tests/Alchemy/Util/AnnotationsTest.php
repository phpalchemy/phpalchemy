<?php
include __DIR__ . '/Fixtures/User.php';

/**
 * Generated by PHPUnit_SkeletonGenerator on 2012-06-05 at 10:33:34.
 */
class AnnotationsTest extends \PHPUnit_Framework_TestCase
{
    /**
     * @var Annotations
     */
    protected $object;

    /**
     * Sets up the fixture, for example, opens a network connection.
     * This method is called before a test is executed.
     */
    protected function setUp()
    {
        $this->annotations = new Alchemy\Util\Annotations;
    }

    /**
     * Tears down the fixture, for example, closes a network connection.
     * This method is called after a test is executed.
     */
    protected function tearDown()
    {
    }

    /**
     * @covers IronG\Util\Annotations::getClassAnnotations
     */
    public function testGetClassAnnotations()
    {
        $result = $this->annotations->getClassAnnotations('\User');
        // Note.- that reserved doc annotations like author, version, etc. will be ommitted
        //        the user class has @author and @version annotations keys, and them should be ommited

        $expected = array(
            "ChangeTrackingPolicy" => Array(
                0 => 'DEFERRED_IMPLICIT',
                1 => 'DEFERRED_EXPLICIT',
                2 => 'NOTIFY'
            ),
            "InheritanceType" => Array(
                0 => 'JOINED'
            ),
            "DiscriminatorColumn" => Array(
                0 => Array(
                    'name' => 'discr',
                    'type' => ' string '
                )
            ),
            "Table" => Array(
                0 => Array(
                    "name" => 'ecommerce_products',
                    "indexes" => Array(
                        'name' => 'sexarch_idx',
                        'column' => 'name'
                    ),
                    'variant' => false
                )
            )
        );

        $this->assertEquals($expected, $result);
    }

    /**
     * @covers IronG\Util\Annotations::getMethodAnnotations
     */
    public function testGetMethodAnnotations()
    {
        $result = $this->annotations->getMethodAnnotations('\User', 'build');
        // Note.- that reserved doc annotations like author, version, etc. will be ommitted
        //        the build method has @param and @return annotations keys, and them should be ommited

        $expected = Array(
            "Attribute" => Array(
                0 => "firstname",
                1 => "lastname"
            ),

            "Cache" => Array(
                0 => Array(
                    "max_time" => 50
                )
            ),
            "testAll" => Array(
                0 => Array(
                    "bool_var" => false,
                    "int_var" => 12345,
                    "float_var" => 12345.6789,
                    "str_var" => "hello",
                    "str_woq" => "word",
                    "str_wq" => "hello word"
                ),
                1 => Array(
                    "name" => "erik",
                    "age" => 27,
                    "address" => Array(
                        "city" => "La paz",
                        "country" => "Bolivia",
                        "avenue" => "El Prado",
                        "building" => "Alameda",
                        "floor" => 15,
                        "dep_num" => 7
                    ),
                    "phone" => 1234567890
                )
            ),
            "Description" => Array(
                0 => "Your system 升级难道每次都要卸载重装？"
            )
        );

        $this->assertEquals($expected, $result);
    }

    /**
     * @expectedException InvalidArgumentException
     */
    public function testErrorMissingCloseBrace()
    {
        $result = $this->annotations->getMethodAnnotations('\User', 'errFunc1');
    }

    /**
     * @expectedException InvalidArgumentException
     */
    public function testErrorMissingCloseQuote()
    {
        $result = $this->annotations->getMethodAnnotations('\User', 'errFunc2');
    }

    /**
     *
     */
    function testComplexString()
    {
        $result = $this->annotations->getMethodAnnotations('\User', 'shouldworks');
        $expected = array(
            "sample" => array(
                0 => array(
                    "err_var" => "1 + 1 = 2, 2+2 = 4",
                    "test_var" => "log text, {0}={1} to params...",
                    "sample" => array(
                        "a"=> 1,
                        "b"=> 2,
                        "c"=> 3
                    )
                )
            )
        );
        $this->assertEquals($expected, $result);
    }
}
